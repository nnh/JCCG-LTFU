...
source("./programs/generate_graph.R", encoding = "UTF-8")
```
...
```{r, echo=F}
RatesAll02(ds.all02.bf.0[ds.all02.bf.0$性別 == "男" , ])
```
### ALL02フォローアップ率,全施設, female
```{r, echo=F}
RatesAll02(ds.all02.bf.0[ds.all02.bf.0$性別 == "女" , ])
```
## ALL02フォローアップ率, 地区別,データ固定時の年齢21歳未満
```{r, echo=F}
data.frame(area, f.u.rate.chiku)
```
...
```{r, echo=F}
data.frame(area, f.u.rate.chiku1)
```
