...
#地区コードマージする
ds.all02.bf.1 <- merge(ds.all02.bf.0, area.cd, by.x = "SITEID", by.y = "code_9digit", all = T)  # ds.all02.bf.1は1252例

#解析対象集団（データ固定2年前以前に死亡した人を除いた集団）の作成・計算
ds.all02.bf <- ds.all02.bf.1[ds.all02.bf.1$no.death.before.2y ==T & ds.all02.bf.1$date.end.trt != "", ] #  ds.all02.bfは962例
