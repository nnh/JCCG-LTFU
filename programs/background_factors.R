...
#facititiesの詳細
number.by.facilities <- table(ds.all02.bf$hospital.type)
shisetsu <- xtabs( ~ hospital.type + SCSTRESC, data = ds.all02.bf)
shisetsu.mat <- matrix(shisetsu, nrow(shisetsu), ncol(shisetsu))
rownames(shisetsu.mat) <- rownames(shisetsu)
shisetsu.mat.0 <- ifelse(shisetsu.mat != 0, 1, 0)
count.facilities <- apply(shisetsu.mat.0, 1, sum)
